...
- **Aşama 29: Caption Uzunluğu & Detaylı Döküm Düzeltmesi (Mart 2026):** Telegram caption 1024 karakter sınırını aşan "message caption is too long" hatası giderildi. `buildDistributionSummary()` dökümleri artık PDF'ten önce ayrı metin mesajı olarak gönderiliyor; PDF caption'ı kısa tutuldu. Plastik iş emrinde stokNot/not sütunundaki "dış alım" gibi eşanlamlı Türkçe metinler otomatik filtrelendi. Kumaş sipariş PDF dosyası `Kumas_Siparisi_*` → `Zakaz_Tkani_*` olarak yeniden adlandırıldı.
...
- **Aşama 30: Code Quality Refactor & GitHub Synchronization (Mart 2026):** Local çalışma ortamı GitHub `main` branch ile senkronize edildi (fast-forward merge). Yapay zeka ajanının (Sandaluci Asistan QA) raporu doğrultusunda `index.ts` ve `pdf.service.ts` içindeki kullanılmayan (unused) değişkenler (örn. `marinaLang`, `isDraft`, `deptMsg` vb.) ve `catch (e)` bloklarındaki gereksiz parametreler temizlendi. Tüm sistem ESLint (`npx eslint .`) ve TypeScript compiler (`npx tsc --noEmit`) denetimlerinden **0 hata (0 error)** ile geçecek şekilde modernize edildi.
