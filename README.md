...
- **Aşama 20: Akıllı Sesli Komut & Whisper V3 (Mart 2026):** Telegram sesli mesajları için `VoiceService` tamamen yenilendi. **Whisper-large-v3** altyapısı ile otomatik dil algılama (Auto-Detect) yeteneği eklendi. VPS bağlantı kısıtlamalarına karşı (Groq 403 Hatası) **Otomatik OpenRouter Fallback** mekanizması devreye alındı.
...
- **Aşama 24: Kendi Kendini Onarma & Self-Cleanup (Mart 2026):** Sistemin gereksiz test verilerinden ve geçici dosyalardan kurtulması için `/temizlik` komutu eklendi. `SelfCleanupService` ile geçici ses dosyaları, test scriptleri ve loglar tek komutla (Boss yetkisiyle) temizlenebilir hale getirildi.
...
- **Model İsmi:** Her zaman `google/gemini-3-flash-preview` veya `.env`'de tanımlı güncel modeli kullanın.
